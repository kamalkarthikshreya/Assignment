...
router.post('/', protect, upload.single('file'), async (req, res) => {
    try {
        ...
        const hasAllColumns = requiredColumns.every((col) =>
            Object.keys(firstRow).some((key) => key.trim().toLowerCase() === col.toLowerCase())
        );
        ...
        const normalizedData = rawData.map((row, idx) => {
            ...
            const find = (name) => keys.find((k) => k.trim().toLowerCase() === name.toLowerCase());
            const firstName = row[find('FirstName')]?.toString().trim();
            const phone = row[find('Phone')];
            const notes = row[find('Notes')]?.toString().trim() || '';

            if (!firstName) throw new Error(`Row ${idx + 2}: FirstName cannot be empty`);
            if (isNaN(phone) || phone === '') throw new Error(`Row ${idx + 2}: Phone must be a valid number`);
